...
1. **Get a token first.** `POST /api/auth/token` with `{"tenant_id": "...", "role": "ADMIN"}` returns a signed `access_token` (HS256, `JWT_SECRET_KEY`, 24h expiry). This is explicitly *not* real credential verification — there's no account/password behind it, just a claims-signing endpoint standing in for a real IdP until one exists. Every other route trusts the signature, not the caller's word.
