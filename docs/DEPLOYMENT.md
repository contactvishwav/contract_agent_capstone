...
- **Local dev (`docker compose up --build backend worker`)**: the same class of stale-connection issue described below for `ui`/Caddy against a recreated `backend` in production can also surface locally against the dev `ui` container's Vite proxy - observed intermittently (roughly 1 in 4 fresh rebuilds in one investigation session, not on every rebuild) as the very first request after a rebuild (e.g. the first upload) hanging or 404ing even though `backend` itself reports healthy; an immediate retry with no other changes succeeded every time it was seen. If you hit this, `docker compose restart ui` before retrying clears it - not required routinely, just worth knowing about before a live demo so a cold-start hang doesn't look like a broken app.
...
- **Backups**: Aura Free includes automated backups on Neo4j's side. The `pdf_source_data` volume contains encrypted original legal PDFs and is durable evidence-bearing data: production needs a tested encrypted backup/restore and capacity policy before source-PDF citations can be released. Redis here is a cache/broker only (LLM response cache, Celery queue, usage counters) - nothing in it needs backing up; losing it just means a cold cache, not data loss.
