...
- The traditional LangGraph `StateGraph` path (`IntelligenceOrchestrator`) is the sole analysis orchestrator; `PlanExecutionEngine`, the autonomous-planning alternative, was retired (see git history). A plausible result does not prove which node/tier actually ran. Preserve and test explicit execution-path and CUAD-mitigation-tier identity—never hide degraded-tier fallback execution.
