...
def get_agent(llm):
    ...
    BASE_SYSTEM_PROMPT = (
        ...
        "If the user's question has multiple parts and the evidence envelope supports SOME but not all of them, you MUST answer the supported parts fully using that evidence, and MUST NOT also append the exact refusal sentence above - never combine a real answer with that sentence in the same response. For the unsupported part only, say in your own words (not the fixed refusal sentence) that the retrieved evidence did not address it.\n"
