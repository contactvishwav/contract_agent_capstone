...
def _fake_intelligence(processing_complete=True, node_status=None, risk_level="LOW"):
    return SimpleNamespace(
        ...
        # See domain/entities.py's ContractIntelligence and tasks.py's
        # _intelligence_to_response_dict.
        quality_grade={}, escalated=False, analysis_method=None,
        execution_path="langgraph_traditional_explicit", planned_execution=False,
    )
...
class TaskProcessesSuccessfullyTests(unittest.TestCase):
    def test_real_worker_eager_execution_returns_expected_shape(self):
        ...
        self.assertEqual(payload["execution_path"], "langgraph_traditional_explicit")
        self.assertFalse(payload["planned_execution"])
